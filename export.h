#ifndef __MLOOP_ML_H__
#define __MLOOP_ML_H__
...
#if !defined(PART_OF_MLOOP) && \
    !defined(STATIC_LINK_MLOOP) && \
    !defined(DYNAMIC_LINK_MLOOP)
#define PART_OF_MLOOP
#endif

#if defined(PART_OF_MLOOP)
#define MLLIB_PRIVATE(x) PRIVATE x
#define MLLIB_PUBLIC(x) PUBLIC x
#elif defined(STATIC_LINK_MLOOP)
#define MLLIB_PRIVATE(x)
#define MLLIB_PUBLIC(x) PUBLIC x
#elif defined(DYNAMIC_LINK_MLOOP)
#define MLLIB_PRIVATE(x)
#define MLLIB_PUBLIC(x) EXTERNAL x
#else
#error Must specify linkage for mloop
#define MLLIB_PRIVATE(x)
...
MLLIB_PUBLIC(void reader_updateOffset_bytesConsumed (Int64 x0, Int64 x1);)
MLLIB_PUBLIC(void reader_setup ();)
MLLIB_PUBLIC(Int32 reader_nextVal ();)
MLLIB_PUBLIC(void writer_init (CPointer x0);)
MLLIB_PUBLIC(void writer_setup ();)
MLLIB_PUBLIC(void mloop_write (CPointer x0, CPointer x1);)
